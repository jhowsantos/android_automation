Then("Left Unit picker value should be Foot") do
  ...
  When("I type {string} to on application keyboard") do |target|
    digits = target.split("")

    digits.each do |num|
      find_element(id: "keypad").
  ...
  Then("I see {string} as current unit converter") do |current_unit|
  ...
  Then("I verify that {int}st result in history list is {string}") do |index, text|
  ...
  When("I press delete from history as {int}st row") do |index|
    parent_element = find_element(id: "history_conversion_list")
    array_of_elements = parent_element.find_elements(id: "history_single_line")
